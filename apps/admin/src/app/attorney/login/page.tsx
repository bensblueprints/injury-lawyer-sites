...
export default function AttorneyLoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setError("");
    const res = await signIn("attorney", {
      email,
      password,
      redirect: false,
      callbackUrl: "/attorney/dashboard",
    });
    setLoading(false);
    if (res?.error) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-900 px-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="text-white text-2xl font-bold">Attorney Client Portal</h1>
          <p className="text-slate-400 text-sm mt-1">Sign in to view your leads</p>
        </div>
