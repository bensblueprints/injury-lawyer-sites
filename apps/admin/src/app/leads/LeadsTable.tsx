...
function statusBadgeClass(status: string) {
  switch (status) {
    ...
    case "SOLD": return "bg-green-900 text-green-300";
    case "ARCHIVED": return "bg-slate-700 text-slate-400";
    default: return "bg-slate-700 text-slate-400";
  }
}

function leadTypeBadge(leadType: string | null) {
  switch (leadType) {
    case "FORM_FILL":
      return <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-slate-700 text-slate-300">Form Fill</span>;
    case "AI_CALL":
      return <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-purple-900 text-purple-300">AI Call</span>;
    case "HOT_TRANSFER":
      return <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-orange-900 text-orange-300">Hot Transfer</span>;
    default:
...
function StatusSelect({ leadId, initialStatus }: { leadId: string; initialStatus: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${statusBadgeClass(status)}`}>
        {status}
      </span>
      <select
        value={status}
...
export function LeadsTable({ leads, filter }: { leads: Lead[]; filter?: string }) {
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-700">
              ...
              <th className="text-left text-slate-400 text-xs font-medium px-4 py-3">Case Type</th>
              <th className="text-left text-slate-400 text-xs font-medium px-4 py-3">Lead Type</th>
              <th className="text-left text-slate-400 text-xs font-medium px-4 py-3">Site</th>
              <th className="text-left text-slate-400 text-xs font-medium px-4 py-3">Attorney</th>
              ...
              <th className="text-left text-slate-400 text-xs font-medium px-4 py-3">Msg</th>
              <th className="text-left text-slate-400 text-xs font-medium px-4 py-3">Call</th>
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 && (
              <tr>
                <td colSpan={10} className="text-slate-500 text-sm text-center py-10">
                  No leads found
                </td>
              </tr>
            )}
            {filtered.map((lead) => (
              <>
                <tr key={lead.id} className="border-b border-slate-700/50 hover:bg-slate-700/20 transition-colors">
                  ...
                  <td className="px-4 py-3 text-slate-300 text-sm">{lead.caseType}</td>
                  <td className="px-4 py-3">{leadTypeBadge(lead.leadType)}</td>
                  <td className="px-4 py-3">
                    {lead.site ? (
                      <div>
                        <div className="text-white text-xs font-medium">{lead.site.city}, {lead.site.state}</div>
                        <div className="text-slate-500 text-xs">{lead.site.practiceArea}</div>
                        <div className="text-slate-600 text-xs">{lead.domain}</div>
                      </div>
                    ) : (
                  ...
                  <td className="px-4 py-3 text-slate-400 text-xs">
                    {lead.site?.attorney?.name ?? <span className="text-slate-600">—</span>}
                  </td>
                  <td className="px-4 py-3">
                ...
                {expandedId === lead.id && lead.message && (
                  <tr key={`${lead.id}-msg`} className="border-b border-slate-700/50 bg-slate-700/30">
