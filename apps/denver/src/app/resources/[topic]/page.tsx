...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://devnercoloradoinjurylawyer.com/resources/${content.slug}`,
    },
    openGraph: {
      title: content.metaTitle,
      description: content.metaDescription,
      url: `https://devnercoloradoinjurylawyer.com/resources/${content.slug}`,
    },
  };
}
        ...
        <div className="grid lg:grid-cols-3 gap-10">
          <article className="lg:col-span-2">
            ...
            <div className="mt-10">
              <CTABox
                config={siteConfig}
                headline="Need Legal Help After an Injury in Denver?"
                body="Free consultation with an experienced Colorado injury attorney. No fee unless we win."
              />
            </div>
            ...
            {relatedResources.length > 0 && (
              <section className="mt-10">
                <h2 className="text-xl font-black text-gray-900 mb-4">
                  More Colorado Injury Law Resources
                </h2>
                <RelatedPages
                  pages={relatedResources.map((p) => ({
                    title: p.label,
                    href: p.href,
                    excerpt: `Helpful guidance for Colorado injury victims on ${p.label.toLowerCase()}.`,
                  }))}
                />
              </section>
            )}
